...
**Consequences.**
- **Migration 0012 (NEW; waits for M0.5 build sprint).** New table `document_cases`; ALTER `documents` ADD `case_id uuid REFERENCES document_cases(case_id) ON DELETE SET NULL` (nullable, indexed). RLS mirrors `documents` (signed-in worker reads own; service role writes). Schema design locked in `docs/planning/M0.5-spec.md`; SQL applied at M0.5-BUILD-01 sprint start.
...
- **What this ADR explicitly does NOT cover.** Migration 0012 SQL (M0.5-BUILD-01 sprint); 10-sec proximity grouping heuristic (M1); multi-image bulk upload UI (M1); case archival policy (Phase 1+); cross-worker case sharing (out of scope, by privacy posture); case state machine triggers (M0.5-BUILD captures DB CHECK constraint shape; M1 may add triggers for derived states).
