...
## ADR-014 — Document cases as primary evidence-organization model

- **Date:** 2026-05-01 (Sprint UX-FLOW-AUDIT)
- **Status:** Accepted (after paradigm validation against 8 plan docs)

**Context.** ADR-013 ratified upload-first fact capture with per-document CLASSIFY/ROUTE/EXTRACT. Sprint B1.13's classification smoke test today (`Contract3.jpeg` — page 12 of a contract, classified as `other` confidence 0.95) surfaced that the per-document unit of classification is wrong for the real-world worker behaviour Apete demonstrates: workers receive contracts and statements as fragments — one page from the employer today, two more next week — and the page on its own genuinely doesn't look like a contract. The classifier worked perfectly; it was asked the wrong question.

The corrected mental model — captured in `docs/document-case-paradigm-v01.md` (ChatGPT critique 2026-05-01) — is that PayChecker is not a document classifier but a document organizer. The smallest semantically meaningful unit is the **document case** — a living object that contains 1+ pages uploaded over time, has a worker-confirmed label, and tracks completion status. Classification suggests the case label; the worker confirms or overrides; calculation continues to operate on confirmed facts (per ADR-001) regardless of which case produced them. `document-intelligence-plan-v01.md` §"Layer 4 — Cross-document reconciliation" already conceptualized this multi-page-across-uploads idea as a *transient* runtime signal; this ADR makes it a *persistent* data model.

**Options.**
- **(a) Pure document-case + deprecate per-document classification.** Cleanest. Discarded — every uploaded image still needs initial classification before it can be merged into (or split from) a case; per-image classification is the input to grouping, not a competing pattern.
- **(b) Document-case as primary; per-document classification preserved as the input to grouping; calc engine unaffected.** Cases own the worker-facing label, the completion status, the storage organization, and the routing decision. Per-image classification (existing CLASSIFY step) becomes the heuristic that proposes case membership. Worker confirms at the case level. Calculation continues to read confirmed facts from `*_facts` tables — it never sees cases. **(Chosen.)**
- **(c) Defer cases until M1+; keep per-document model in M0.5.** Discarded — every M0.5 row would need a future case-id backfill migration; the data-model commitment is cheap now and expensive later.
- **(d) Document-case replaces fact-level confirmation.** Discarded — violates ADR-001 (confirmation model is sacred at fact level). Case label confirmation is a NEW orthogonal confirmation; it does NOT replace fact-level confirmation. Each extracted field still walks the existing `proposed → confirmed` flow per `confirmation-flow.md` + Migration 0010 triggers.

**Decision.** Option (b). PayChecker's evidence-organization model is built around `document_cases`, not individual document files. A case is a living object representing one logical document (e.g., a contract, a payslip period) which may contain pages uploaded across multiple sessions. **Classification, extraction, storage organization, and worker label-confirmation operate at the case level.** Calculation continues to operate at the confirmed-fact level (ADR-001 unchanged). The case state machine (`draft → suggested → confirmed → partial → complete`) is **orthogonal to** the fact state machine (`proposed → confirmed`). Confirming a case label does NOT auto-confirm extracted facts; each fact still walks the existing confirmation flow.

**Reasoning.**
- **Aligns with ADR-013.** Pipeline shape (UPLOAD → CLASSIFY → ROUTE → EXTRACT → ADR-012's 5 stages) is preserved; the unit of CLASSIFY/ROUTE shifts from per-document to per-case (where case = 1+ documents). Extraction still runs per-document; the extracted fields are attached to the case via `documents.case_id`.
- **Aligns with ADR-001.** Confirmation model is sacred AT FACT LEVEL. Cases add a *new* confirmation surface (the worker confirms the case's label), but each underlying fact still requires its own confirm tap before it becomes calc-eligible. Provenance enum (`worker_entered`, `ocr_suggested`, `ocr_suggested_confirmed`) is unchanged.
- **Aligns with ADR-003.** Worker-facing vocabulary (Ready / Needs more pages / Can still use / Not sure yet) is calculator-language not legal-language. Forbidden terms (Failed/Validation/Confidence/Classification/Error) cannot leak from this layer.
- **Aligns with ADR-007.** Two gates → three gates (per ADR-013) extend cleanly: gate-1 (case label confirmed) → gate-2 (extracted facts confirmed per ADR-012) → gate-3 (comparison reviewed). The gates are sequential, not concurrent.
- **Aligns with personas.md.** ESL-friendly; ≥48px tap targets; "this is what we found, never you should"; no dropdowns; no confidence numbers worker-facing. Apete-shaped from day 1.
- **Adjacent to Layer 4 memory architecture.** `document-intelligence-plan-v01.md` §Layer 4 already named the cross-document reconciliation concept as a runtime signal. This ADR persists that signal as the `document_cases` table. Vector-similarity reconciliation continues to run at classification time as the heuristic that proposes case membership.

**Consequences.**
- **Migration 0014 (applied 2026-05-01 in Sprint M0.5-BUILD-01; originally drafted as "0012").** New table `document_cases`; ALTER `documents` ADD `case_id uuid REFERENCES document_cases(case_id) ON DELETE SET NULL` (nullable, indexed). RLS mirrors `documents` (signed-in worker reads own; service role writes). Plus `classify_with_case()` RPC for atomic case INSERT + document linkage. Schema body locked in `docs/planning/M0.5-spec.md`. Numbering shifted from 0012 → 0014 because Migrations 0012 (`fk_indexes_and_rls_perf`) and 0013 (`storage_rls_public_pattern`) had already shipped between Migration 0011 and the M0.5 build window.
- **`api/classify.ts` input shape changes.** Accepts an array of `document_id`s (a case) instead of a single id; falls back to per-image when array length = 1. Output schema unchanged at the per-case level (still `detected_type + confidence`). Downstream `extraction-service-v01.md` schemas unchanged. Versioning bump: `classify-prompt-v01` → `classify-prompt-v02`.
- **PLAN-PRJ-mvp-phases.md gains an M0.5 sub-phase.** Smallest-useful slice between Phase 0 stocktake item 11 and item 12. Locked to **Option B (minimum-with-cases):** singleton-case-per-upload, case-card UI, status vocabulary, "+ Add more pages" path, no 10-sec proximity grouping (deferred to M1).
- **Vocabulary lock.** Worker-facing statuses: Ready / Needs more pages / Can still use / Not sure yet. Internal DB enum (`document_cases.completion_status`): `draft`, `suggested`, `confirmed`, `partial`, `complete`. The renderer maps internal enum → worker-facing label.
- **UI surface ownership.** `/upload` page redesigned to render upload zone + case-card list inline. No new `/cases` route in M0.5. Direct deep-link `/upload/case/{case_id}` resolves to the same page scrolled to the relevant card. M1 may evolve to a separate index when case volume justifies it.
- **`documents.batch_id` preserved unchanged.** Batch = upload session (transient grouping at upload time); case = logical document (may span ≥1 batches). Different axes; both retained.
- **integration-plan-v01.md ADR placeholders renumbered.** That doc reserved ADR-014/015/016 for POL-004 (Calculation Explanation) / POL-005 (Worker Context) / POL-006 (Sentiment) on 2026-04-29. Document-case ratified first (today); integration POLs shift +1 → ADR-015 / ADR-016 / ADR-017. Updated in this commit.
- **Cross-references.** ADR-001 (confirmation sacred — case label is a NEW confirmation surface, NOT a replacement for fact-level confirmation); ADR-003 (info not advice — vocabulary lock enforces calculator-language); ADR-005 (indexing not looping — case retrieval is by case_id, never full case-history scan); ADR-006 (orient don't collect — UPLOAD copy still carries the orient; case state machine adds zero up-front collection); ADR-007 (two gates → three gates per ADR-013, three gates → still three with case-label confirmation folded into gate-1); ADR-012 (5 stages survive; SUGGEST + CONFIRM stages now apply at the case level for the label, and at the fact level for each extracted field); ADR-013 (extended by this ADR; per-document CLASSIFY/ROUTE shifts to per-case CLASSIFY/ROUTE).
- **Reversibility.** Single `DROP TABLE document_cases CASCADE` rolls back the new table; `documents.case_id` keeps NULL with a code-only check. Per-document classification path of ADR-013 continues to work without `document_cases` present (singleton-case = direct per-document semantics). No one-way doors.
- **What this ADR explicitly does NOT cover.** 10-sec proximity grouping heuristic (M1); multi-image bulk upload UI (M1); case archival policy (Phase 1+); cross-worker case sharing (out of scope, by privacy posture); case state machine triggers (M0.5-BUILD-01 omitted these; M1 may add triggers for derived states).
- **Pressure test.** SKILL-PRJ-pressure-test.md run (5/5 cleared) — see `docs/document-case-paradigm-v01.md` for full Apete-misreading + reversibility + privacy pass; this ADR records the binding decision.
